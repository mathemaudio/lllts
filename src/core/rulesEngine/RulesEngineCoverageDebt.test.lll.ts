...
export class RulesEngineCoverageDebtTest {
	...
	static async fullCoverageMapsToZeroDebt(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
	...
	static async ninetySixCoverageNotice(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
	...
	static async ninetyFiveCoverageWarningBoundary(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
	...
	static async ninetyCoverageWarning(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
	...
	static async eightyFiveCoverageAlertBand(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
	...
	static async eightyCoverageErrorBoundary(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
	...
	static async seventyCoverageOverHundredDebt(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
	...
	static async zeroClassesStatus(subjectFactory: SubjectFactory<unknown>, scenario: ScenarioParameter) {
		const input = scenario.input
		const assert: AssertFn = scenario.assert
		const waitFor: WaitForFn = scenario.waitFor
